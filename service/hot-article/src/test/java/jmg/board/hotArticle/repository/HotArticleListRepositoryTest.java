package jmg.board.hotArticle.repository;
...
class HotArticleListRepositoryTest {
    ...
    void addTest() throws InterruptedException {
        LocalDateTime time = LocalDateTime.of(2025, 11, 18, 0, 0);
        long limit = 3;
        ...
        hotArticleListRepository.add(5L, time, 5L, limit, Duration.ofSeconds(3));

        List<Long> articleIds = hotArticleListRepository.readAll("20251118");

        assertThat(articleIds).hasSize(Long.valueOf(limit).intValue());
        assertThat(articleIds.get(0)).isEqualTo(5);
        assertThat(articleIds.get(1)).isEqualTo(4);
        assertThat(articleIds.get(2)).isEqualTo(2);

        TimeUnit.SECONDS.sleep(5);

        assertThat(hotArticleListRepository.readAll("20251118")).isEmpty();
    }
}